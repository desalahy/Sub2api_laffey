#!/bin/bash
#
# Laffey API Installation Script
# Laffey API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/desalahy/Sub2api_laffey/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="desalahy/Sub2api_laffey"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="Laffey API 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 Laffey API 服务："
    ...
    ["not_installed"]="Laffey API 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 Laffey API。"
    ...
    ["uninstall_complete"]="Laffey API 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 Laffey API"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 Laffey API"
    ...
    ["server_config_desc"]="配置 Laffey API 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="Laffey API Installation Script"
    ...
    # Completion
    ["install_complete"]="Laffey API installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start Laffey API service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading Laffey API..."
    ...
    ["not_installed"]="Laffey API is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove Laffey API from your system."
    ...
    ["uninstall_complete"]="Laffey API has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install Laffey API"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove Laffey API"
    ...
    ["server_config_desc"]="Configure Laffey API server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        "$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+-laffey\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="sub2api_laffey_${version_num}_${OS}_${ARCH}.tar.gz"
...
[Unit]
Description=Laffey API - AI API Gateway Platform
Documentation=https://github.com/desalahy/Sub2api_laffey
...
# Upgrade function
upgrade() {
    # Check if Laffey API is installed
    if [ ! -f "$INSTALL_DIR/sub2api" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+-laffey\.[0-9]+' || echo "unknown")
...
# Requires: Laffey API must already be installed
install_version() {
